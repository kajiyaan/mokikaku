...
├── voice.html            … ご家族の声（掲載同意取得済み）
├── flow.html             … ご利用の流れ（企業/障がい者の縦型ステップ）
├── faq.html              … よくある質問
├── entry.html            … エントリーフォーム（企業/障がい者タブ切替）
├── company.html          … 会社概要＋お問い合わせ
├── privacy.html          … プライバシーポリシー
├── send.php              … フォーム送信処理（info@mokikaku.jp 宛にメール通知）
├── images/
│   ├── support-worker.jpg … 就労サポートの様子（4:3 / 1200x900）
│   └── workplace.jpg      … 職場環境（4:3 / 1200x900）
├── css/
...
### サイト構成（ナビゲーション）
トップ / 企業様向けサービス / 障がいをお持ちの方へ / ご家族の声 / ご利用の流れ / よくある質問 / 会社概要 ＋ エントリー（CTAボタン）
※ プライバシーポリシーはフッターのみ

### 画像の扱い
スマホ写真（3000x4000程度）をそのまま置くと重いため、Pillowで **4:3にトリミング → 1200x900 / quality 82** に変換して `images/` に配置している。元ファイルはリポジトリに残さない（Git履歴には保持）。
...
- ✅ 「障害」→「障がい」表記統一
- ✅ エントリー／お問い合わせフォームの実送信（send.php でメール通知）
- ✅ プライバシーポリシーページ作成・全フォームから接続
- ✅ 会社概要を実データ化（所在地・代表者・対応エリア・協力パートナー）
- ✅ 実写真の掲載（就労サポート／職場環境）
- ✅ ご家族の声を専用ページ voice.html として公開（掲載同意取得済み）
- ✅ 著作権表示を 2026 に更新
- ✅ **仮データの全撤去**（電話番号・FAX・資本金の行、実績数値セクションを削除）

### 今後のTODO（未対応）
- ⬜ **実績数値の復活**: 導入企業数・支援実績などのセクションは実態がないため削除済み。CSS（`.numbers*`）は残してあるので、実績が出たらHTMLを戻すだけで復活できる。
- ⬜ **ご家族の声の追加**: 現在1件。2件目以降が集まったらカードを縦に並べる構成へ組み替え可能。属性（例「20代男性のご家族」）を添えると読み手が重ねやすい。
- ⬜ **電話番号**: 現在サイト上に電話番号の掲載なし（メールのみ）。番号を載せる場合は company.html と entry.html に追加。

### 掲載物の同意管理（重要）
- ご家族の声（voice.html）… ご本人・ご家族の掲載同意取得済み。匿名化して掲載（氏名・就職先企業名・日付は不掲載）。
- 就労中の方が写った写真（images/support-worker.jpg）… 掲載にあたり同意を得ている前提で公開中。
- 今後、体験談や写真を追加する際は **必ず本人（未成年なら保護者）の同意を得ること**。障がいに関する情報は個人情報保護法上の「要配慮個人情報」にあたり、また創作した体験談の掲載はステマ規制（景品表示法）違反となり得る。

---

## 10. 関連プロジェクト

同じさくらアカウント内で、**アビリティスタッフ株式会社**（協力パートナー・`kajiyaan/abilitystaff`）のサイトも同じ仕組みで構築予定。
- さくらの認証情報は共通（`kzshop.sakura.ne.jp` / ユーザー `kzshop`）
- アップロード先は `/home/kzshop/www/abilitystaff/`
- Secrets は本リポジトリと同じ3つを、abilitystaff リポジトリ側にも登録する
- 本ファイルと `DEPLOY_SETUP.md` がそのまま構築手順の参考になる
